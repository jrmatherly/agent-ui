...
### Manual Installation with mise (Recommended)
...
3. Tools install automatically on project entry. Start the development server:
...
mise validate     # Run all checks (lint + format + typecheck)
mise test         # Run unit tests (Vitest)
mise test:e2e     # Run E2E tests (Playwright)
mise release      # Create a release
...
```bash
mise test:e2e:install   # or: pnpm test:e2e:install
```

This downloads the Chromium browser required for testing. See [Developer Guide](./docs/DEVELOPER_GUIDE.md) for full testing documentation.
