...
# 2. Updating package.json version
# 3. Creating and pushing a git tag
# 4. Creating a GitHub release to trigger docker-images.yml workflow
#
# Usage:
#   mise release              # Auto-increment patch version (interactive)
#   mise release v1.2.3       # Use specific version
#   mise release:major        # Increment major version
#   mise release:minor        # Increment minor version
#   mise release:patch        # Increment patch version
...
update_package_json() {
    ...
    # Use node to update version (preserves formatting better than sed)
    node -e "
        const fs = require('fs');
    ...
    echo -e "${BLUE}Updated package.json version to ${version}${NC}"
}
