...
const InlineCode: FC<PreparedTextProps> = ({ children }) => {
  return (
    <code className="bg-background-secondary/50 relative rounded-xs p-1 whitespace-pre-wrap">
      {children}
    </code>
  )
}
...
const TableHead = ({ className, ...props }: TableHeaderProps) => (
  <thead
    className={cn(
      className,
      'border-border rounded-md border-b bg-transparent p-2 text-left text-sm font-semibold'
    )}
    {...filterProps(props)}
  />
)
